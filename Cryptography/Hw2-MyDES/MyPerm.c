...
/********Definition of My Permutation Init Functions(Modified)********/
...
/*******Definition of Pointers to My Permutation Table(Modified)******/
...
/***************My Permutation Init Functions(Modified)***************/
static void perm_init(char perm[8][256][8],char p[64]){
   register int b,j,k;
   int i,m;
   ...
   for(i=0;i<8;i++){
      for(j=0;j<256;j++){
         for(k=0;k<64;k++){
            b=p[k]-1;
            if((b/8)!=i) continue;
            if(!(j&bytebit[b%8])) continue;
            m=k%8;                                            
            perm[i][j][k/8]|=bytebit[m]; 
         }
      }
   }
}
...
int des_init(int mode){
   if(sbox_output_perm_table!=NULL) return 1;
   des_mode=mode;
