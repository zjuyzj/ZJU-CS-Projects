...
/***********Change:Use Table plaintext_32bit_expanded_to_48bit_table***********/
/**************A straight-forward implementation of function f()***************/
#ifndef notdef
#define notdef
#endif
/******************************************************************************/
...
#ifndef compf
/**************My straight-forward implementation of function f()***************/
static long32 f(ulong32 r,unsigned char subkey[8]) {
    int i,j;
    unsigned char s[4],plaintext[8],m[4],t[4],nth_bit,bit_value,row,col;
    unsigned char* p_uchar;
    unsigned long int rval;
    //Step 1(Already Adjust to Big Endian outside f())
    p_uchar=(unsigned char*)&r;
    for(i=0;i<4;i++) s[3-i]=*(p_uchar++);
    //Step 2
    for(i=0;i<8;i++) plaintext[i]=0;
    for(i=0;i<8;i++){
        for(j=0;j<6;j++){
            nth_bit=plaintext_32bit_expanded_to_48bit_table[i*6+j]-1;
            bit_value=((s[nth_bit/8]&bytebit[nth_bit%8])!=0);
            plaintext[i]|=(bit_value<<(5-j));
        }
    }
    //Step 3
    for(i=0;i<8;i++) plaintext[i]^=subkey[i];
    //Step 4
    for(i=0;i<4;i++) m[i]=0;
    for(i=0;i<8;i++){
        row=((plaintext[i]&0x20)>>4)|(plaintext[i]&0x01);
        col=(plaintext[i]&0x1E)>>1;
        if(i%2!=0) m[i/2]|=sbox[i][row*16+col];
        else m[i/2]|=(sbox[i][row*16+col]<<4);
    }
    //Step 5
    for(i=0;i<4;i++) t[i]=0;
    for(i=0;i<32;i++){
        nth_bit=sbox_perm_table[i]-1;
        bit_value=((m[nth_bit/8]&bytebit[nth_bit%8])!=0);
        t[i/8]|=(bit_value<<(7-i%8));
    }
    //Step 6(Save on Big Endian)
    p_uchar=(unsigned char*)&rval;
    for(i=0;i<4;i++) *(p_uchar++)=t[3-i];
    return rval;
}
/******************************************************************************/
#endif
