...
void des_encrypt_key(unsigned char *p, unsigned char *mykey){
   des_init(0);
   des_set_key(mykey);
   des_encrypt((char *)p);
   des_done();
...
int main(void){
   ...
   A69AFFEA65AB11E3
   */
   unsigned char rc4_seed_key[9], des_seed_iv[9], des_seed_key[9];
   unsigned char des_iv[9],  des_key[9];
   rc4_key k;
...
/*************************************My DES_CFB Function**************************************/
void des_cfb_encrypt(unsigned char p[],int n,unsigned char des_seed_iv[],unsigned char des_iv[],unsigned char des_seed_key[]){
    unsigned char i,j,rc4_xor[8],des_key[8],des_cipher[8];
    unsigned char des_cfb_cipher[0x100];
    for(i=0;i<8;i++){
        rc4_xor[i]=des_iv[i]^des_seed_iv[i];
        des_key[i]=des_seed_key[i]^rc4_xor[i];
    }
    for(i=0;i<n;i++){
        for(j=0;j<8;j++) des_cipher[j]=des_iv[j];
        des_encrypt_key(des_cipher,des_key);
        des_cfb_cipher[i]=des_cipher[0]^p[i];
        for(j=0;j<7;j++) des_iv[j]=des_iv[j+1];
        des_iv[7]=des_cfb_cipher[i];
    }
    for(i=0;i<n;i++) printf("%02X",des_cfb_cipher[i]);
    printf("\n");
    return;
}
/***********************************My DES_CFB Function End************************************/